...
This guide covers how to set up the development environment and build Escriba from source across different platforms.
...
```bash
git clone git@github.com:AlejandroAP9/Escriba.git
cd Escriba
```
...
The raw binary (`src-tauri/target/release/escriba`) cannot run standalone — it needs Tauri resource files (tray icons, sounds, VAD model) to be co-located at the expected path.

**Install from the deb bundle** (works on any Linux distro):

```bash
cd /tmp
ar x /path/to/Escriba/src-tauri/target/release/bundle/deb/Escriba_*_amd64.deb data.tar.gz
tar xzf data.tar.gz
sudo cp usr/bin/escriba /usr/bin/
sudo cp -a usr/lib/. /usr/lib/
sudo cp -r usr/share/icons/hicolor/* /usr/share/icons/hicolor/
sudo cp usr/share/applications/Escriba.desktop /usr/share/applications/
sudo ldconfig
```
...
```bash
sudo cp src-tauri/target/release/escriba /usr/bin/
...
```
Bundling Escriba_*_amd64.AppImage
...
~/.cache/tauri/linuxdeploy-x86_64.AppImage --appimage-extract-and-run \
  --appdir Escriba.AppDir --plugin gtk --output appimage
```
...
this because it builds from a short root such as `D:\a\Escriba`.)
...
Alternatively, clone the repo to a short root (e.g. `C:\Escriba`).
